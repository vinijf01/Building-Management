<x-app-layout>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <section class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    ...
                    <div class="grid grid-cols-[160px_1fr] gap-x-3 gap-y-2 text-sm">
                        ...
                        <div>{{ $booking->start_date->format('Y-m-d') }} → {{ $booking->end_date->format('Y-m-d') }}
                        </div>
                    </div>
                </section>
                ...
                <section class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    ...
                    @if ($payment->proof_image)
                        ...
                        <img src="{{ asset('storage/' . $payment->proof_image) }}" alt="Payment proof"
                            class="w-full rounded-lg border border-gray-200 object-contain max-h-64 mb-3">

                        <button id="openProofModal" type="button"
                            class="bg-yellow-600 text-white px-4 py-2 rounded-lg font-semibold">
                            Re-upload proof
                        </button>
                    @else
                        <p class="text-gray-500 mb-3">
                            Upload a clear photo/screenshot of your bank transfer receipt.
    ...
    {{-- Modal --}}
    <div id="proofModal"
        class="hidden fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-auto">
                ...
                <div class="p-4 flex flex-col gap-3 overflow-auto">
                    ...
                    <div class="flex items-center gap-3">
                        <button id="browseBtn" type="button"
                            class="bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold">Browse image…</button>
                    ...
                    <div id="previewWrap" class="hidden flex flex-col gap-1">
                        ...
                        <img id="previewImg" src="" alt="Preview"
                            class="w-full rounded-lg border border-gray-200 object-contain max-h-64">
                    </div>
                </div>

                <div class="flex justify-end gap-3 p-4 border-t border-gray-200">
                    <button type="button" id="cancelBtn" class="btn-secondary btn-sm">Cancel</button>
                    <button id="sendProofBtn" type="submit"
                        class="bg-green-500 text-white px-4 py-2 rounded-lg font-semibold" disabled>Send proof</button>
                </div>
            </form>
        </div>
    </div>
